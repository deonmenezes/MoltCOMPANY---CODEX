...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    .from('community_bots')
    .select('id, name, bot_name, description, icon_url, author_name, bot_role, color, upvotes, downvotes, character_file, soul_md, created_at, category, tags, likes, deploys, fork_count')
    .eq('status', 'published')

  // Search by name, description, or role
  if (search.trim()) {
    query = query.or(`name.ilike.%${search}%,description.ilike.%${search}%,bot_role.ilike.%${search}%,bot_name.ilike.%${search}%`)
  }
  ...
  return NextResponse.json({
    bots: (bots || []).map(bot => ({
      ...bot,
      role: bot.bot_role || null,
      view_count: bot.likes || 0,
      deploy_count: bot.deploys || 0,
      fork_count: bot.fork_count || 0,
    })),
    total: count || 0,
  })
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if ((count || 0) >= 3) {
      return NextResponse.json(
        { error: 'You can post a maximum of 3 public job packs' },
        { status: 400 }
      )
    }
    ...
    if (!name?.trim() || !character_file?.trim()) {
      return NextResponse.json(
        { error: 'Job title and onboarding brief are required' },
        { status: 400 }
      )
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to publish job pack' }, { status: 500 })
    }
