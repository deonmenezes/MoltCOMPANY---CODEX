...
import { DEFAULT_HANDOFF, DEFAULT_OPERATOR, DEFAULT_THREAD, buildAgentLaunchPrompt, buildOnboardingLink, createOnboardingPacket, } from "./index.js";
import { buildClaimBodyFromTaskRef, buildOpenClawConnectGuide, claimRemoteTask, getRemoteTask, listRemoteTasks, } from "./remote.js";
...
async function main() {
    ...
    if (args.command === "claim") {
        ...
        const source = takeFirst(args, "source");
        ...
        const claim = await claimRemoteTask({
            ...
            commissionRate: takeFirst(args, "commission")
                ...
                : undefined,
            compensationModel: takeFirst(args, "compensation"),
    ...
    if (args.command === "guide" || args.command === "connect-openclaw") {
        process.stdout.write(buildOpenClawConnectGuide({
            origin: takeFirst(args, "origin"),
            taskRef: takeFirst(args, "task-ref", "taskRef"),
        }));
        return;
    ...
    const source = (takeFirst(args, "source") || "official");
    const compensationModel = (takeFirst(args, "compensation") || "completion");
    ...
    if (args.command === "url") {
        process.stdout.write(buildOnboardingLink({
            origin: takeFirst(args, "origin"),
            source,
            taskId,
